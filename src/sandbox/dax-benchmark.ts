...
async function runDiskProbe(sandbox: any, timeout: number): Promise<Pick<DaxIterationResult, 'diskSeqWriteMbps' | 'diskSeqReadMbps' | 'diskFsyncLatencyMs'>> {
...
async function runCpuProbe(sandbox: any, timeout: number): Promise<{ cpuOpsPerSec: number; cpuStealPercent?: number; cpuInfo: DaxCpuInfo }> {
  const result = await withTimeout(
 ...
 */
async function runPauseResumeProbe(
  sandbox: any,
  timeout: number,
